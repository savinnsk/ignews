        ...
        const user = await fauna.query<User>(
            q.Get(
                q.Match(
                    ...
                    q.Casefold(session.user.email) // getting data from fauna
                )
            )
        )

        let custumerId = user.data.stripe_customer_id // getting stripe id from faunadb
